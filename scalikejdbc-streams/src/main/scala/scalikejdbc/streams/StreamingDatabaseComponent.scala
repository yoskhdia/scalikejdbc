...
trait StreamingDatabaseComponent extends LogSupport {
  type Executor <: AsyncExecutor
  type StreamingActionContext <: StreamingContext

  trait DatabaseContainer {
    def executor: StreamingDB.Executor
    def name: Any
    def connectionPoolContext: DB.CPContext
    def settings: SettingsProvider
    def bufferNext: Boolean
  }
  ...
  protected def acquireSession(context: StreamingContext, database: DatabaseContainer): Unit = {
    context.currentSession = connect(database.name, database.connectionPoolContext, database.settings).readOnlySession()
  }

  protected def releaseSession(context: StreamingContext, discardErrors: Boolean): Unit = {
  ...
  private[this] def scheduleSynchronousStreaming(database: DatabaseContainer, a: StreamingAction[_, _ <: WithExtractor], ctx: StreamingActionContext)(initialState: a.State): Unit = try {
  ...
  def createAction[A, E <: WithExtractor](sql: StreamingSQL[A, E]): StreamingAction[A, E] = {
    new StreamingAction[A, E] {
      override protected[this] def createInvoker(): StreamingInvoker[A, E] = new StreamingInvoker[A, E] {
        override protected[this] def streamingSql: StreamingSQL[A, E] = sql
      }
    }
  }

  def createPublisher[A, E <: WithExtractor](action: StreamingAction[A, E], database: DatabaseContainer): DatabasePublisher[A] = {
    new DatabasePublisherImpl[A, E](action, database)
  }

  def createContext(database: DatabaseContainer, subscriber: Subscriber[_]): StreamingActionContext

  protected class DatabasePublisherImpl[A, E <: WithExtractor](action: StreamingAction[A, E], database: DatabaseContainer) extends DatabasePublisher[A] with LogSupport {
    override def subscribe(s: Subscriber[_ >: A]): Unit = {
      ...
      val context = createContext(database, s)
      val subscribed = try {
  ...
  class StreamingContext(subscriber: Subscriber[_], database: DatabaseContainer, val bufferNext: Boolean) extends Subscription with LogSupport {
    /**
     ...
     */
    @volatile private[StreamingDatabaseComponent] var sync = 0

    private[StreamingDatabaseComponent] var currentSession: DBSession = null
     ...
     */
    private[StreamingDatabaseComponent] var deferredError: Throwable = null

    /**
     * The state for a suspended streaming action. Must only be set from a synchronous action
     * context.
     */
    private[StreamingDatabaseComponent] var streamState: AnyRef = null

    /** The streaming action which may need to be continued with the suspended state */
    private[StreamingDatabaseComponent] var streamingAction: StreamingAction[_, _ <: WithExtractor] = null
    ...
    def restartStreaming(): Unit = {
      ...
      if (s ne null) {
        ...
        scheduleSynchronousStreaming(database, a, this.asInstanceOf[StreamingActionContext])(s.asInstanceOf[a.State])
      } else {
